...
/**
 * Serializes and writes the BCFF global header to a file stream.
 *
...
void write_bcff_header(bcff_header_t *h, FILE *f) {
    ...
    memcpy(h->magic, "BCFF", 4);
    memcpy(buf, h->magic, 4);

    buf[4] = BCFF_CURRENT_VERSION;
    buf[5] = h->algorithm;
...
/**
 * Serializes and writes the BCFF frame header to a file stream.
